...
from feed_forward.decoder import Decoder
from feed_forward.encoder import Encoder
from generic.aligner import AlignmentNetwork
from generic.pos_encoding import PositionalEncoding
from glow_tts.duration_predictor import DurationPredictor
import torch.nn as _nn
class BaseTTS(_nn.Module):
    def __init__(self, config, ap=None, tokenizer=None, speaker_manager=None, *a, **k):
        super().__init__(); self.config=config; self.speaker_manager=speaker_manager; self.num_speakers=1; self.embedded_speaker_dim=0
        self.args=getattr(config,"model_args",None) or getattr(config,"args",config)
    def init_multispeaker(self, config, *a, **k):
        self.num_speakers=getattr(self.args,"num_speakers",1) or 1
        self.embedded_speaker_dim=0

from helpers import average_over_durations, expand_encoder_outputs, generate_attention, sequence_mask
SpeakerManager=object
TTSTokenizer=object
plot_alignment=plot_avg_energy=plot_avg_pitch=plot_spectrogram=lambda *a,**k: None
...
class ForwardTTS(BaseTTS):
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None}):  # pylint: disable=unused-argument
        ...
        """
        g = self._set_speaker_input(aux_input)
        # dynamic mask: inference is a single full sequence (no padding), so the
        # mask is all-ones of the live length -- avoids baking x.shape[1] under tracing.
        x_mask = torch.ones(x.shape[0], 1, x.shape[1], dtype=x_emb.dtype if False else torch.float32, device=x.device)
        # encoder pass
