"""ScaledAdam optimizer used by the ZipVoice recipe.

ScaledAdam scales each parameter's update by the parameter's own RMS norm
and clips per-batch update magnitude (``clipping_scale``), which the
Zipformer line of work pairs with the Eden schedule (Yao et al.,
arXiv:2310.11230; Zhu et al., arXiv:2506.13053). Ported from
k2-fsa/ZipVoice (Apache-2.0).
"""
...
import torch
def fix_random_seed(seed):  # lhotse.utils.fix_random_seed (test-only use)
    import random
