"""ZipVoice flow-matching TTS model.

The ZipVoice architecture (Zhu et al., "ZipVoice: Fast and high-quality
zero-shot text-to-speech with flow matching", arXiv:2506.13053): a
TTSZipformer text encoder producing a per-frame text condition, and a
Zipformer flow-matching decoder regressing the straight-line vector field
over Vocos log-mel features, with in-context (infilling) speaker prompting
and classifier-free-guidance text-condition dropout. Ported from
k2-fsa/ZipVoice (Apache-2.0), imports made package-relative.
"""
...
from phoonnx_train.zipvoice.solver import EulerSolver
from phoonnx_train.zipvoice.zipformer import TTSZipformer
from phoonnx_train.zipvoice.common import (
    condition_time_mask,
    get_tokens_index,
    make_pad_mask,
    pad_labels,
